...
Canon is cloned with EMORI's managed SSH identity only when its checkout is
absent; the Agent System source checkout remains optional. Run `install` again
when that state changes. `doctor` checks whether the installed state still
matches. Use `install --skip-setup` only for installation-only automation.
