...
that state changes. The prefix also links Canon as the `tanaab` plugin. `doctor`
checks whether the installed state still matches. Use `install --skip-setup` only
for installation-only automation.
