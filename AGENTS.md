...
- `dist/` is generated publish output for hosting and release preparation; do not hand-edit it during normal work and do not commit local `dist/` rebuilds for ordinary source, docs, examples, or PR CI fixes.
- `site/llms.txt` is the source for hosted `/llms.txt`; `dist/llms.txt` is generated by `scripts/build-dist.js` and should not be hand-edited.
- Keep `/llms.txt` concise. Update `site/llms.txt` when README or hosted-doc changes alter EMORI's purpose, bootstrap flow, support boundaries, primary resources, or public install/usage contract; skip it for minor wording, changelog-only, or internal-development edits.
...
- Treat `bun run build` as CI-owned by default. Do not run it locally for ordinary `boot.sh`, docs, examples, or PR CI fixes. Only run it when the user explicitly asks for a local build or the task is directly about release or hosted `dist/` output.
- If `bun run build` is run accidentally during normal work, discard the generated `dist/` changes instead of committing them unless the user explicitly asks to include generated output.
