...
# should generate public key fixtures for envvar authorized-key installation
rm -f .tmp/id_emori_envvars_authorized_file .tmp/id_emori_envvars_authorized_file.pub
rm -f .tmp/id_emori_envvars_authorized_raw .tmp/id_emori_envvars_authorized_raw.pub
ssh-keygen -t ed25519 -N "" -C "emori-envvars-authorized-file@example.test" -f .tmp/id_emori_envvars_authorized_file >/dev/null
ssh-keygen -t ed25519 -N "" -C "emori-envvars-authorized-raw@example.test" -f .tmp/id_emori_envvars_authorized_raw >/dev/null
...
EMORI_AUTHORIZED_KEY='file:.tmp/id_emori_envvars_authorized_file.pub' \
EMORI_AUTHORIZED_KEYS="$(cat .tmp/id_emori_envvars_authorized_raw.pub),op://omfsw2uztmi2xqpid5g3kiv6ba/id_test" \
...
# should install the primary ssh key filename from envvars
...
# should not duplicate authorized keys
...
if [[ -f "$HOME/.ssh/authorized_keys" ]]; then
  grep -vxF \
    -e "$(cat .tmp/id_emori_envvars_authorized_file.pub)" \
    -e "$(cat .tmp/id_emori_envvars_authorized_raw.pub)" \
    -e "$(cat id_test.pub)" \
